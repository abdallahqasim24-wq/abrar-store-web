// server.js — ESM, طلبات متعددة + مبيعات منفصلة + ربح بدون الشحن
// ===============================================
...
app.use((req, res, next) => {
  ...
  res.locals.fixImg = (u) => {
    ...
    } catch { return "https://placehold.co/80x80?text=No+Img"; }
  };
  next();
});
...
// -------- Helpers --------
// الربح بدون الشحن: (qty*sale) - (qty*cost)
const profitOf = (s) =>
...
app.get("/", async (_req, res) => {
  ...
  const lowStock = (
    await query(`
      ...
      WHERE stock<=5
      ORDER BY stock ASC, updated_at DESC NULLS LAST, created_at DESC LIMIT 12`)
  ).rows;
  ...
  uploader(req, res, async (err) => {
    try {
      ...
      for (const f of (req.files?.images||[]))
        await query(`INSERT INTO product_images(product_id,url) VALUES($1,$2)`, [ins.rows[0].id, `/public/uploads/${f.filename}`]);
...
app.post("/products/:id/update", (req, res, next) => {
  ...
  uploader(req, res, async (err) => {
    try {
      ...
      if (main) image_path = `/public/uploads/${main.filename}`;
      await query(
        `UPDATE products SET name=$1,brand=$2,category=$3,cost_price=$4,sale_price=$5,stock=$6,image_path=$7,updated_at=NOW() WHERE id=$8`,
...
// -------- Sales page (نموذج طلب + قائمة الطلبات + المبيعات) --------
app.get("/sales", async (_req, res) => {
  ...
  // البنود المسلّمة = المبيعات
  const deliveredSales = (await query(
    `SELECT s.*, p.name AS product_name, p.image_path AS product_image
     FROM sales s JOIN products p ON p.id=s.product_id
...
// إضافة بيع مفرد (يبقى بند مستقل ليس ضمن طلب)
app.post("/sales", async (req, res) => {
  const { product_id, quantity, sale_price, cost_price, shipping_cost, note,
          customer_name, customer_phone, customer_city } = req.body;
  const p = (await query(`SELECT * FROM products WHERE id=$1`, [Number(product_id)])).rows[0];
  if (!p) return res.redirect("/sales");
  ...
  await query(
    ...
    [p.id, qty, Number(sale_price||p.sale_price), Number(cost_price||p.cost_price),
     Number(shipping_cost||0), note||"", (customer_name||"").trim(),
     (customer_phone||"").trim(), (customer_city||"").trim()]
  );
  await query(`UPDATE products SET stock=GREATEST(0,stock-$1), updated_at=NOW() WHERE id=$2`, [qty, p.id]);
  res.redirect("/sales");
});

// إضافة عدة بنود دفعة واحدة (يبنيها كبنود مستقلة)
  ...
  try {
    ...
    for (let i=0; i<n; i++){
      ...
      await query(
        `INSERT INTO sales(product_id,quantity,sale_price,cost_price,shipping_cost,note,customer_name,customer_phone,customer_city,delivery_status)
         VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'pending')`,
        [pid,qty,sp,cp,sh,nt,(customer_name||"").trim(),(customer_phone||"").trim(),(customer_city||"").trim()]
      );
      await query(`UPDATE products SET stock=GREATEST(0,stock-$1), updated_at=NOW() WHERE id=$2`, [qty, pid]);
    }
    res.redirect("/sales");
  } catch { res.redirect("/sales"); }
});
...
app.post("/sales/:id/update", async (req, res) => {
  ...
  await query(
    `UPDATE sales SET product_id=$1, quantity=$2, sale_price=$3, cost_price=$4, shipping_cost=$5, note=$6,
                      customer_name=$7, customer_phone=$8, customer_city=$9 WHERE id=$10`,
    [Number(product_id||old.product_id), Number(quantity||old.quantity), Number(sale_price||old.sale_price),
     ...
     (customer_city??old.customer_city)||"", id]
  );
  res.redirect("/sales");
});
app.post("/sales/:id/delivery", async (req, res) => {
  ...
  await query(
    `UPDATE sales SET delivery_status=$1, delivered_at=CASE WHEN $1='delivered' THEN NOW() ELSE NULL END WHERE id=$2`,
    [status, id]
  );
  res.redirect("back");
});
...
app.post("/orders", async (req, res) => {
  const { customer_name, customer_phone, customer_city, note,
          product_id=[], quantity=[], sale_price=[], cost_price=[], shipping_cost=[], item_note=[] } = req.body;
  const ins = await query(
...
app.get("/orders/:id", async (req, res) => {
  ...
  const items = (await query(
    `SELECT s.*, p.name AS product_name, p.image_path AS product_image
     FROM sales s JOIN products p ON p.id=s.product_id
     WHERE s.order_id=$1 ORDER BY s.id ASC`, [id]
  )).rows;
  const products = (await query(
    `SELECT id,name,stock,cost_price,sale_price,image_path FROM products ORDER BY name`
  )).rows;
...
app.post("/orders/:id/items/bulk-update", async (req,res)=>{
  ...
  for (let i=0;i<n;i++){
    ...
    if (sid && pid){
      await query(
        `UPDATE sales SET product_id=$1,quantity=$2,sale_price=$3,cost_price=$4,shipping_cost=$5,note=$6 WHERE id=$7 AND order_id=$8`,
        [pid, Math.max(1,Number([].concat(quantity)[i]||1)), Number([].concat(sale_price)[i]||0),
         Number([].concat(cost_price)[i]||0), Number([].concat(shipping_cost)[i]||0),
         String([].concat(item_note)[i]||""), sid, id]
      ); continue;
    }
    if (!sid && pid){
      await query(
...
app.post("/orders/:id/items/:itemId/delete", async (req,res)=>{
  await query(`DELETE FROM sales WHERE id=$1 AND order_id=$2`, [Number(req.params.itemId), Number(req.params.id)]);
...
app.post("/orders/:id/status", async (req,res)=>{
  ...
  if (apply_to_items==="on"){
    await query(
      `UPDATE sales SET delivery_status=$1, delivered_at=CASE WHEN $1='delivered' THEN NOW() ELSE NULL END WHERE order_id=$2`,
      [status, id]
    );
  }
  res.redirect(`/orders/${id}`);
...
app.get("/reports", async (req,res)=>{
  ...
  if (range==="monthly"){
    ...
    rows = (await query(
      `SELECT s.*,p.name AS product_name,p.image_path AS product_image
       FROM sales s JOIN products p ON p.id=s.product_id
       WHERE TO_CHAR(s.sold_at,'YYYY-MM')=$1 ORDER BY s.sold_at DESC`, [ym]
    )).rows;
  } else {
    const y = Number(year)||Number(dayjs().tz(TZ_NAME).format("YYYY"));
    const m = Number(month)||Number(dayjs().tz(TZ_NAME).format("MM"));
    ...
    rows = (await query(
      `SELECT s.*,p.name AS product_name,p.image_path AS product_image
       FROM sales s JOIN products p ON p.id=s.product_id
       WHERE DATE(s.sold_at)=DATE($1) ORDER BY s.sold_at DESC`, [ds]
    )).rows;
  }
  res.render("reports", {
    title, rows, range,
...
// -------- Reports PDF (تحميل كسول لـ puppeteer) --------
app.get("/reports/pdf", async (req,res,next)=>{
  try{
    ...
    try { ({ default: puppeteer } = await import("puppeteer")); }
      ...
      return res.end(`<p style="font-family:Arial">ميزة PDF تحتاج تثبيت <b>puppeteer</b>.<br>نفّذ: <code>npm i puppeteer</code></p>`);
    }

    // أعِد استخدام منطق /reports
    const { range="daily", year, month, day } = req.query;
    let rows=[], title="";
    if (range==="monthly"){
      const y = Number(year)||Number(dayjs().tz(TZ_NAME).format("YYYY"));
