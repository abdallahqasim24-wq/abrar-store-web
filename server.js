// server.js — يدعم الطلبات متعددة المنتجات
// ===================================================
...
async function query(sql, params=[]) {
  ...
  try { const res = await client.query(sql, params); return res; }
...
async function initDb() {
  ...
  // orders (طلب واحد يجمع عدة منتجات)
  await query(`
...
app.post("/products/:id/stock", async (req,res)=>{
  await query(`UPDATE products SET stock = GREATEST(0, stock + $1), updated_at=NOW() WHERE id=$2`, [Number(req.body.delta||0), Number(req.params.id)]);
  res.redirect("/products");
});
...
app.post("/products/bulk-delete", async (req,res)=>{
  const ids = (Array.isArray(req.body.ids)?req.body.ids:String(req.body.ids||"").split(",")).map(n=>Number(n)).filter(Boolean);
  if (ids.length) await query(`DELETE FROM products WHERE id = ANY($1::int[])`, [ids]);
  res.redirect("/products");
});

// -------- Sales (لا يزال يعمل كالسابق) --------
...
app.post("/sales", async (req,res)=>{
  ...
  await query(`UPDATE products SET stock = GREATEST(0, stock - $1), updated_at=NOW() WHERE id=$2`, [qty, prod.id]);
...
app.post("/orders", async (req,res)=>{
  const {
    ...
    product_id = [], quantity = [], sale_price = [], cost_price = [], shipping_cost = [], item_note = []
  } = req.body;
...
app.get("/orders/:id", async (req,res)=>{
  ...
  // Products for adding more
  const products = (await query(`SELECT id, name, stock, cost_price, sale_price, image_path FROM products ORDER BY name`)).rows;

  const revenue = items.reduce((a,s)=>a + Number(s.sale_price)*Number(s.quantity), 0);
  const profit  = items.reduce((a,s)=>a + ((Number(s.sale_price)-Number(s.cost_price))*Number(s.quantity) - Number(s.shipping_cost||0)), 0);
...
// تغيير حالة الطلب ككل (ينسخها اختياريًا لكل البنود)
...
app.get("/reports/pdf", async (req,res,next)=>{
  try{
    ...
    const totalProfit  = rows.reduce((a,s)=>a + (
      (Number(s.sale_price)*Number(s.quantity)) - (Number(s.cost_price)*Number(s.quantity)) - Number(s.shipping_cost||0)
    ), 0);

    const html = await new Promise((resolve,reject)=>{
      app.render("report-pdf", { title, rows, totalRevenue, totalProfit, dayjs }, (err,str)=>{
        if (err) return reject(err); resolve(str);
      });
    });
